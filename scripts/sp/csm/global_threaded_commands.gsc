...
CMD_CHANGEMAP_f( arg_list )
{
	...
	if ( map != "" )
	{
		...
		level scripts\sp\csm\_com::com_printf( "g_log", "cmdinfo", "Changemap Usage: " + self.playername + " changed map to " + display_name, self );
		level scripts\sp\csm\_com::com_printf( "say|con", "notitle", message, self );
		rotation_string = "map " + map;
		setDvar( "sv_maprotationCurrent", rotation_string );
		for ( i = level.custom_commands_restart_countdown; i > 0; i-- )
		{
			level scripts\sp\csm\_com::com_printf( "con|say", "notitle", i + " seconds" );
			wait 1;
		}
...
CMD_ROTATE_f( arg_list )
{
	...
	level scripts\sp\csm\_com::com_printf( "g_log", "cmdinfo", "Rotate Usage: " + self.playername + " rotated the map.", self );
	level scripts\sp\csm\_com::com_printf( "say|con", "notitle", message, self );
	for ( i = level.custom_commands_restart_countdown; i > 0; i-- )
	{
		wait 1;
		level scripts\sp\csm\_com::com_printf( "con|say", "notitle", i + " seconds left" );
	}
	level notify( "end_commands" );
	wait 0.5;
	exitLevel( false );
...
CMD_RESTART_f( arg_list )
{
	...
	level scripts\sp\csm\_com::com_printf( "g_log", "cmdinfo", "Restart Usage: " + self.playername + " restarted the map.", self );
	level scripts\sp\csm\_com::com_printf( "say|con", "notitle", message, self );
	for ( i = level.custom_commands_restart_countdown; i > 0; i-- )
	{
		wait 1;
